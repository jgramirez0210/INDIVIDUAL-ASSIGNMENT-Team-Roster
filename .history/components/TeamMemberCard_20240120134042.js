  ...
  return (
    <Card style={{ width: '18rem', margin: '10px' }}>
      ...
      <Card.Body>
        ...
        <p className="card-text bold">{teamMemberObj && teamMemberObj.roll}</p>
        {/* DYNAMIC LINK TO VIEW THE AUTHOR DETAILS  */}
        <Link href={teamMemberObj ? `/author/${teamMemberObj.firebaseKey}` : '#'} passHref> <Button variant="primary" className="m-2">VIEW</Button>
        </Link>
        {/* DYNAMIC LINK TO EDIT THE BOOK DETAILS  */}
        <Link href={teamMemberObj ? `/author/edit/${teamMemberObj.firebaseKey}` : '#'} passHref>
          <Button variant="info">EDIT</Button>
         </Link>
        <Button ariant="danger" onClick={deleteThisAuthor} className="m-2">
          DELETE
        </Button>
      </Card.Body>
    </Card>
