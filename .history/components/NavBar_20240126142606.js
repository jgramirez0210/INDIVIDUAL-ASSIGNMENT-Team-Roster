...
function NavBar() {
  ...
  useEffect(() => {
    if (value.length > 0) {
      fetch('https://team-roster-300f5-default-rtdb.firebaseio.com/team.json')
        ...
        .then(responseData => {
          ...
          for (const key in responseData) {
            if (responseData.hasOwnProperty(key)) {
              const teamName = responseData[key].name.toLowerCase();
              if (teamName.includes(searchQuery)) {
                setResults(prevResults => [...prevResults, responseData[key].name]);
              }
            }
          }
        })
